...
extension PHImageManager {
    typealias HandlePotentialDegradedResultCompletion<T> = (Result<Media.DisplayRepresentation<T>, Error>) -> Void

    static func handlePotentialDegradedResult<T>(_ result: (item: T?, info: [AnyHashable : Any]?),
                                                 _ completion: @escaping HandlePotentialDegradedResultCompletion<T>) {
